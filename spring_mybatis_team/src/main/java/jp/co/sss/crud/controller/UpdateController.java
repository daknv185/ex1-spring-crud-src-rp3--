...
public class UpdateController {
	...
	public String inputUpdate(Integer empId, @ModelAttribute EmployeeForm employeeForm) {
		...
		Employee SearchEmpId = searchForEmployeesByEmpIdService.execute(empId);

		//TODO 検索した社員情報をformに積め直す(BeanCopyクラスを用いてもよい)
		BeanCopy.copyEntityToForm(SearchEmpId, employeeForm);
	...
	public String completeUpdate(EmployeeForm employeeForm, HttpSession session) {
		...
		Employee CopyForm = BeanCopy.copyFormToEmployee(employeeForm);

		// TODO 権限がnullの場合、デフォルトの権限を設定
		if (CopyForm.getAuthority() == null) {
			CopyForm.setAuthority(Constant.DEFAULT_AUTHORITY);
		}

		// TODO 社員情報を更新する
		updateEmployeeService.execute(CopyForm);
